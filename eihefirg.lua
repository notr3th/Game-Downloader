...
farmingTab:CreateToggle({
    ...
    Callback = function(Toggle)
        ...
        if autoBuy then
            ...
            while autoBuy and task.wait(0.5) do
...
farmingTab:CreateToggle({
    ...
    Flag = "autoSpin",
    Callback = function(Toggle)
        autoSpinEnabled = Toggle
        if Toggle then
            task.spawn(function()
                while autoSpinEnabled and autoBuy do
                    task.wait(1)

                    local spinWheel = Workspace:FindFirstChild("RainbowSpinWheels")
                    local wheel1 = spinWheel and spinWheel:FindFirstChild("1")
                    local overhead = wheel1 and wheel1:FindFirstChild("Overhead")
                    local countdown = overhead and overhead:FindFirstChild("Countdown")

                    if countdown and countdown:IsA("TextLabel") and countdown.Text == "SPIN NOW" then
                        walkTo(Vector3.new(-433.952, -7.002, 78.927))

                        local success, err = pcall(function()
                            game:GetService("ReplicatedStorage")
                                :WaitForChild("Packages")
                                :WaitForChild("Net")
                                :WaitForChild("RE/RainbowSpinWheelService/Spin")
                                :FireServer()
                        end)
