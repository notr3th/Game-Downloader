...
function sendLog(Message)
    if not logCooldown and webhookURL ~= "" then
        logCooldown = true

        local avatarUrl = Players:GetUserThumbnailAsync(LocalPlayer.UserId, Enum.ThumbnailType.HeadShot, Enum.ThumbnailSize.Size420x420)
        local data = {
        ...
        -- Supports most executor environments including Fluxus
                ...
                for _, Animal in ipairs(MovingAnimals:GetChildren()) do
                    ...
                    if animalHumanoidRootPart and (animalHumanoidRootPart.Position - HumanoidRootPart.Position).Magnitude <= 10 then
                        ...
                        if checkWhitelist(animalHumanoidRootPart) then
                            spawn(function()
                                sendLog("Item purchased: [Item Name]")
                            end)
