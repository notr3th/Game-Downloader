...
local function sendWebhook(message)
    ...
    -- Use pcall to avoid errors if HTTP is blocked
    pcall(function()
        -- If running in executor, HttpPost usually works
                ...
                for _, Animal in ipairs(MovingAnimals:GetChildren()) do
                    ...
                    if animalHumanoidRootPart and (animalHumanoidRootPart.Position - HumanoidRootPart.Position).Magnitude <= 10 then
                        ...
                        if checkWhitelist(animalHumanoidRootPart) then
                            ...
                            local mutation = Overhead.Mutation.Text
                            
                            local message = string.format("Bought: %s (Rarity: %s, Mutation: %s)", displayName, rarity, mutation)
                            sendWebhook(message)
                        end
                    end
                end
...
local webhookInput = farmingTab:CreateInput({
    Name = "Webhook URL",
    CurrentValue = "",
