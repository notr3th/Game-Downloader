...
function sendLog(Message)
    if not logCooldown then
        ...
        local Data = {
            ...
            avatar_url = "https://api.newstargeted.com/roblox/users/v1/avatar-headshot?userid=" .. LocalPlayer.UserId .."&size=150x150&format=Png&isCircular=false",
            content = Message
        }
                ...
                for _, Animal in ipairs(MovingAnimals:GetChildren()) do
                    ...
                    if animalHumanoidRootPart and (animalHumanoidRootPart.Position - HumanoidRootPart.Position).Magnitude <= 10 then
                        ...
                        if checkWhitelist(animalHumanoidRootPart) then
                            spawn(function()
                                sendLog("```diff\n- Character purchased: " .. animalHumanoidRootPart.Info.AnimalOverhead.DisplayName.Text .. "```")
                            end)
